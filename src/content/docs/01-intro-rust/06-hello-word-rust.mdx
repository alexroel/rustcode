...
1. **Compilar el Código Rust**
   ...
   :::note
   Cuando compiles un programa Rust, el compilador generará un archivo ejecutable que puedes ejecutar en tu sistema operativo. Pero puede que necesites instalr `build-essential` en Linux para compilar el programa.

   Para instalar `build-essential` en Ubuntu, usa el siguiente comando:
...
:::tip[¡Y eso es todo!]
Has creado, compilado y ejecutado tu primer programa en Rust. Ahora que tienes una base, puedes comenzar a explorar más características y capacidades del lenguaje Rust.
:::